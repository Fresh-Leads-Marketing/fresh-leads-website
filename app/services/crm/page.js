...
function CaseStudy(){return(<section style={{background:BG2,padding:"70px 24px"}}><div style={{maxWidth:900,margin:"0 auto"}}><FI><p style={{color:B,fontWeight:700,fontSize:13,letterSpacing:".07em",textTransform:"uppercase",marginBottom:24,textAlign:"center"}}>Why CRM matters</p></FI><div className="csg detail-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))",gap:16}}>{[{title:"Centralize your data",desc:"Stop switching between your POS, email, phone system, and spreadsheets. Our CRM pulls everything into one dashboard so you can see every customer interaction in one place.",delay:0},{title:"Automate follow-ups",desc:"Auto-tag customers by behavior, trigger re-engagement campaigns for lapsed visitors, and track every touchpoint from first visit to lifetime loyalty.",delay:0.08},{title:"POS API integration",desc:"We connect directly to your laundry POS system via API so customer data syncs automatically. New customers, visit history, and transaction data flow into your CRM in real time.",delay:0.16},{title:"Automated email & SMS",desc:"Once synced, your CRM triggers personalized messages automatically. Welcome sequences for new customers, win-back campaigns for lapsed visitors, and review requests after every visit.",delay:0.24}].map((card,i)=>(<FI key={i} delay={card.delay}><div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"28px 26px",height:"100%",display:"flex",flexDirection:"column"}}><h3 style={{fontSize:17,fontWeight:700,color:"#fff",marginBottom:12}}>{card.title}</h3><p style={{fontSize:15,color:"rgba(255,255,255,0.6)",lineHeight:1.65,margin:0,flex:1}}>{card.desc}</p></div></FI>))}</div></div></section>)}
...
function FAQ(){const[o,setO]=useState(null);const Q=[["Does the CRM work with my POS system?","Yes. We integrate directly via API with Cents, LaundroWorks, CCI, and CleanCloud. Once connected, customer data syncs automatically in real time — new customers, visit history, and transaction data all flow into your CRM without any manual work on your end."],["Do I have to manage the CRM myself?","No. This is fully managed by our team. We handle all segmentation, campaign creation, automations, reporting, and ongoing optimization. You get the results and the dashboard access, but you never have to touch the backend unless you want to."],["Can you import my existing customer data?","Absolutely. We import from CSV, Excel, or other CRM exports and clean everything during onboarding. Duplicates are merged, formatting is standardized, and your data is organized into actionable segments before we launch any campaigns."],["How long does setup take?","Typically 48 hours after your kickoff call. That includes importing your data, building customer segments, setting up automations, and testing everything end to end. We don't launch until you've reviewed and approved the setup."],["What happens to my data if I cancel?","It's yours. If you ever decide to leave, we export all of your customer data, campaign history, and contact lists in a clean format that you can take to any other platform. No data held hostage."]];return(<section style={{background:BG2,padding:"80px 24px"}}><div style={{maxWidth:700,margin:"0 auto"}}><FI><h2 style={{fontSize:"clamp(24px,3vw,32px)",fontWeight:800,color:"#fff",marginBottom:36,textAlign:"center"}}>Common questions</h2></FI>{Q.map(([q,a],i)=><FI key={i} delay={i*.03}><div onClick={()=>setO(o===i?null:i)} style={{background:o===i?"rgba(43,127,255,0.04)":"rgba(255,255,255,0.03)",border:`1px solid ${o===i?"rgba(43,127,255,0.2)":"rgba(255,255,255,0.06)"}`,borderRadius:12,marginBottom:8,cursor:"pointer"}}><div style={{padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:14}}><h3 style={{fontSize:14,fontWeight:640,color:"#fff",margin:0}}>{q}</h3><span style={{width:24,height:24,borderRadius:6,background:o===i?B:"rgba(255,255,255,0.07)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,color:o===i?"#fff":"rgba(255,255,255,0.3)",fontSize:15}}><span style={{transform:o===i?"rotate(45deg)":"none",transition:"transform .2s",display:"block"}}>+</span></span></div>{o===i&&<div style={{padding:"0 18px 14px"}}><p style={{fontSize:13.5,color:"rgba(255,255,255,0.45)",lineHeight:1.65,margin:0}}>{a}</p></div>}</div></FI>)}</div></section>)}
