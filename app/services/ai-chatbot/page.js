...
function CaseStudy() {
  return (<section style={{ background: BG2, padding: "70px 24px" }}><div style={{ maxWidth: 900, margin: "0 auto" }}><FI><p style={{ color: B, fontWeight: 700, fontSize: 13, letterSpacing: ".07em", textTransform: "uppercase", marginBottom: 24, textAlign: "center" }}>Client results</p></FI><div className="csg detail-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><FI><div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "28px 26px" }}><div style={{ display: "flex", gap: 3, marginBottom: 10 }}>{[1,2,3,4,5].map(s => <span key={s} style={{ color: "#F59E0B", fontSize: 12 }}>★</span>)}</div><p style={{ fontSize: 15, color: "rgba(255,255,255,0.6)", lineHeight: 1.65, marginBottom: 16 }}>"The chatbot booked <span style={{ color: B, fontWeight: 700 }}>214 pickup orders in month one</span> while my staff ran the store. Handles 80% of questions automatically."</p><div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 34, height: 34, borderRadius: 8, background: "rgba(43,127,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: B }}>D</div><div><div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>David R.</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)" }}>FreshPress Laundry · Atlanta, GA</div></div></div><div style={{ display: "flex", gap: 12, marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.06)" }}>{[["Orders booked","214","month one"],["Auto-handled","80%","of inquiries"],["Response","<5s","avg. reply"]].map(([l,v,s],i)=><div key={i} style={{textAlign:"center",flex:1}}><div style={{fontSize:11,color:"rgba(255,255,255,0.3)"}}>{l}</div><div style={{fontSize:18,fontWeight:800,color:i===0?"#22C55E":B}}>{v}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.2)"}}>{s}</div></div>)}</div></div></FI><FI delay={0.08}><div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, padding: "28px 26px" }}><div style={{ display: "flex", gap: 3, marginBottom: 10 }}>{[1,2,3,4,5].map(s => <span key={s} style={{ color: "#F59E0B", fontSize: 12 }}>★</span>)}</div><p style={{ fontSize: 15, color: "rgba(255,255,255,0.6)", lineHeight: 1.65, marginBottom: 16 }}>"Missing <span style={{ color: B, fontWeight: 700 }}>30-40 calls/week</span> after hours. Voice bot catches every one now. Added $6K/month from calls we used to miss."</p><div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ width: 34, height: 34, borderRadius: 8, background: "rgba(43,127,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: B }}>A</div><div><div style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>Angela W.</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)" }}>QuickWash Express · Tampa, FL</div></div></div><div style={{ display: "flex", gap: 12, marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.06)" }}>{[["Calls saved","30-40","/week"],["Revenue","+$6K","/month"],["Capture","100%","after-hrs"]].map(([l,v,s],i)=><div key={i} style={{textAlign:"center",flex:1}}><div style={{fontSize:11,color:"rgba(255,255,255,0.3)"}}>{l}</div><div style={{fontSize:18,fontWeight:800,color:i===1?"#22C55E":B}}>{v}</div><div style={{fontSize:10,color:"rgba(255,255,255,0.2)"}}>{s}</div></div>)}</div></div></FI></div></div></section>);
}
...
function FAQ() {
  ...
  const Q = [["Setup time?","About 1 week. We train on your services, hours, pricing, then test before going live."],["Complex questions?","Handles 80%+ automatically. Complex ones transfer to your team with full context."],["Works with my website?","Yes — any website. Also Facebook, Instagram, and Google Business Profile."],["Frustrated customers?","AI detects frustration and offers a human immediately. Always an option."],["Customizable?","We train it on your brand voice and policies. You approve everything before launch."]];
...
function CTA() {
  return (<section style={{ background: BG2, padding: "90px 24px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}><FI><div style={{ background: "linear-gradient(135deg, rgba(43,127,255,0.1), rgba(43,127,255,0.03))", border: "1px solid rgba(43,127,255,0.18)", borderRadius: 20, padding: "clamp(36px,6vw,56px) clamp(24px,5vw,44px)", textAlign: "center" }}><h2 style={{ fontSize: "clamp(24px,4vw,34px)", fontWeight: 800, color: "#fff", marginBottom: 12 }}>Ready to automate support?</h2><p style={{ fontSize: 15, color: "rgba(255,255,255,0.5)", maxWidth: 440, margin: "0 auto 20px", lineHeight: 1.6 }}>Book a free call — we'll demo the chatbot and voice bot live.</p><a href="/contact" style={{ display: "inline-block", background: B, color: "#fff", padding: "14px 30px", borderRadius: 10, fontWeight: 700, fontSize: 15, textDecoration: "none", boxShadow: "0 4px 20px rgba(43,127,255,0.3)" }}>Schedule Your Free Demo →</a></div></FI></div></section>);
}
...
export default function Page() {
  return (<div style={{ fontFamily: "'DM Sans', -apple-system, sans-serif", background: BG, minHeight: "100vh" }}><Nav /><Hero /><CaseStudy /><Features /><Objections /><FAQ /><CTA /><Footer /></div>);
}
